...
#define VRef 1.8f         // Reference voltage.
#define ADCMaxValue 65535 // Maximum # of bits coming from the gyro and ADC [2^16 - 1].
...
void setup() {
  ...
  Serial.print("Accelerometer range: "); Serial.println(accelGyro.getFullScaleAccelRange());
  Serial.print("Gyroscope range: "); Serial.println(accelGyro.getFullScaleGyroRange());
...
/* Serial.print("Raw accel: [");
   PrintVector(*agX, *agY, *agZ);

  Serial.print("Accel voltages: ");
  PrintVector(XVolts, YVolts, ZVolts);*/
...
/*Serial.print("Final force calc: [");
  PrintVector(XForceInG, YForceInG, ZForceInG);*/
...
void GetGyroAngles(float *gyroX, float *gyroY, float *gyroZ) {
  Serial.print("Gyro: ");
  PrintVector(*gyroX, *gyroY, *gyroZ);
  
//  float GyroSensitivity = 
  
  // TODO Make a gyro sensitivity-getting function.
  // TODO VZero rate for gyro is +/- 20 deg./s. = ? Volts
  RateGyroX = (((*gyroX * (VRef / MaxADCValue)) - VzeroRate) / GyroSensitivity);
  RateGyroY = (((*gyroY * (VRef / MaxADCValue)) - VzeroRate) / GyroSensitivity);
}
...
// Make sure our gyro and accelerometer read in
// the largest possible range of values.
void SetDeviceRanges() {
  accelGyro.setFullScaleAccelRange(3);
  accelGyro.setFullScaleGyroRange(3);
}

// Utility function to print out a 
// vector on Serial output.
void PrintVector(int16_t x, int16_t y, int16_t z) {
  Serial.print("[");
  Serial.print(x, 6);
  Serial.print(", ");
  Serial.print(y, 6);
